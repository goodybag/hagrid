...
, templates = {
  ...
  , create: 'lib/generators/web/templates/create.handlebars'
  , update: 'lib/generators/web/templates/update.handlebars'
  , del:    'lib/generators/web/templates/delete.handlebars'
  }
...
, getResourceMethods = function(resource, urlParts, idVars){
    ...
    for (var i = 0; i < resource.methods.length; i++){
      ...
      if (method === "get"){
        ...
        output += 'create: ' + templates.create({
          idVars: idVars
        , urlParts: urlParts
        });
      } else if (method === "patch" || method === "put"){
        output += 'update: ' + templates.update({
          idVars: idVars
        , urlParts: urlParts
        });
      } else if (method === "delete"){
        output += 'del: ' + templates.del({
          idVars: idVars
        , urlParts: urlParts
        });
      }
...
module.exports = function(document, options, callback){
  ...
  , generateModules: function(){
      ...
      for (var key in resources){
        fs.writeFileSync(options.buildDir + '/resources/' + key + '.js', templates.module({ $return: generate(resources[key]) }));
      }
    }
